...
public class WeatherClient {
    ...
    public String getTodayWeather() {
        ...
        if (!HttpStatus.OK.equals(responseEntity.getStatusCode())) {
            throw new ServerException("날씨 데이터를 가져오는데 실패했습니다. 상태 코드: " + responseEntity.getStatusCode()); // if-else에서 if,if문으로 나눈이유는 다른 예외가 생기면 else문을 계속 써줘야하기때문에 if,if문으로 나눠서함 조건이 맞지않으면 예외를 던지면됨
        }
